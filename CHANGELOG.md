...
- **`cdno templates vars <type>`** (#271) — list the `{{placeholders}}` a note type's template
  ...
  the source. The supplied set is **derived** from the type's built-in template (which references
  exactly the contextual keys the create path fills), so it can't drift from the scaffold; config
  `[variables]` / `[variables.prompt]` names are folded in and classified by source
  (`supplied` / `config` / `prompt`). A `--variant` flag selects a variant template (e.g. `gym` for
  `tracking`); `--json` emits a `{ name, source }` array. A new public `Vault::template_placeholders`
  and `cdno_core::template::placeholder_names` back it.
