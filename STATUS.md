...
**Current release**: 0.1.24 (2026-06-29). The CLI is daily-usable end-to-end and the MCP server is production-ready (41 tools) against Claude Desktop / Claude Code / Kiro / Gemini CLI. v0.1.24 adds `--json` to the `show` verbs (`project`/`portfolio`/`stewardship show`, mirroring the MCP detail DTOs), **closing #227 — every read and write verb now honours `--json`** (only maintenance/interactive/bootstrap commands abstain). Earlier: v0.1.23 (`--json` standalone write commands), v0.1.22 (`--json` write results on project/action/portfolio/stewardship), v0.1.21 (`--json` list verbs), v0.1.20 (`search --json`), v0.1.19 (portfolio↔project links #253), v0.1.18 (`$EDITOR` review prose #230, memoisation #248), v0.1.17 (frontmatter-order lint #236), v0.1.16 (#242/#244/#245). See [`CHANGELOG.md`](CHANGELOG.md) for the full list.
