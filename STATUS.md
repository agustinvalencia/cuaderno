...
| **5 — Tauri UI** | `cdno-tauri` backend, React frontend (Tailwind v4 + shadcn-style, Recharts), Home / Weekly / Commitments views | In progress — M0 (#311), M1 (scaffold, #53), M2 (Home interactions, #54), M3 (global capture + inbox drawer), M4 (Commitments Timeline, #56) and M5 (project detail + Actions view + note reader + command palette) done: the app is daily-usable (orient, start, tick, inline state edit), capture-anywhere via `⌘⇧C`, the full promises view lives at `/commitments`, and every card/link/search hit now navigates somewhere real — `/projects/:slug` (full map with waiting-on writes, milestones, backlinks), `/actions` (cross-project energy-filtered list), a Radix note reader with wikilink navigation, and a `⌘K` palette (search + capture/log verbs); the first shadcn-style overlay primitive (`Sheet`) is vendored under `components/ui/`; next M6 (Weekly Review) |
