...
| **5 — Tauri UI** | `cdno-tauri` backend, React frontend (Tailwind v4 + shadcn-style, Recharts), Home / Weekly / Commitments views | In progress — M0 (watcher + domain gaps, #311) and M1 (scaffold + shell + live orientation, #53) done; views M2+ pending; shadcn-style primitives deliberately not vendored yet (first need is M2) |
