...
| **5 — Tauri UI** | `cdno-tauri` backend, React frontend (Tailwind v4 + shadcn-style, Recharts), Home / Weekly / Commitments views | In progress — M0 (#311), M1 (scaffold, #53), M2 (Home interactions, #54), M3 (global capture + inbox drawer) and M4 (Commitments Timeline, #56) done: the app is daily-usable (orient, start, tick, inline state edit), capture-anywhere via `⌘⇧C`, and the full promises view (chronological, amber-as-accent, collapsed slipped-past, context chips, milestone/commitment completion) lives at `/commitments`; next M5 (project detail + note reader + palette); shadcn-style primitives deliberately not vendored until first overlay need (M5) |
