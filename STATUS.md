...
**Current release**: 0.1.19 (2026-06-28). The CLI is daily-usable end-to-end and the MCP server is production-ready (41 tools) against Claude Desktop / Claude Code / Kiro / Gemini CLI. v0.1.19 makes portfolio ↔ project linking bidirectional: `create_portfolio` with a project now backfills that project map's `## Links`, and a new retrofit verb (`cdno portfolio link --project` + MCP `link_portfolio_to_project`, catalogue 40 → 41) sets the portfolio's `project:` frontmatter and backfills the project's `## Links` (#253). It follows v0.1.18 (`$EDITOR` review prose #230, lint/normalise template memoisation #248), v0.1.17 (frontmatter-order lint #236), and v0.1.16 (config `ignore` globs #242, daily `{{weekday}}` #244, weekly `This Week's Goal` rename #245). See [`CHANGELOG.md`](CHANGELOG.md) for the full list.
