...
**Current release**: 0.1.23 (2026-06-29). The CLI is daily-usable end-to-end and the MCP server is production-ready (41 tools) against Claude Desktop / Claude Code / Kiro / Gemini CLI. v0.1.23 extends `{path, message}` `--json` write results to the standalone write commands (`log`/`capture`/`file`/`track` + `question`/`commit` verbs), so **every write verb now honours `--json`**; only the `show` verbs remain on #227. Earlier: v0.1.22 (`--json` write results on project/action/portfolio/stewardship), v0.1.21 (`--json` list verbs), v0.1.20 (`search --json`), v0.1.19 (portfolio↔project links #253), v0.1.18 (`$EDITOR` review prose #230, memoisation #248), v0.1.17 (frontmatter-order lint #236), v0.1.16 (#242/#244/#245). See [`CHANGELOG.md`](CHANGELOG.md) for the full list.
