...
| **5 — Tauri UI** | `cdno-tauri` backend, React frontend (Tailwind v4 + shadcn-style, Recharts), Home / Weekly / Commitments views | In progress — M0 (#311), M1 (scaffold, #53), M2 (Home interactions, #54) and M3 (global capture + inbox drawer) done: the app is daily-usable (orient, start, tick, inline state edit) and capture-anywhere via `⌘⇧C`; next M4 commitments view; shadcn-style primitives deliberately not vendored until first overlay need (M5) |
