...
impl CuadernoServer {
    ...
    #[tool(
        description = "Start work on an action that is ALREADY on the project map: matches the open bullet by substring `query` and logs `started [[project]] — <bullet>` to today's daily note (an em dash, U+2014 — `current_focus` matches on it exactly, so a line composed by hand with a hyphen is invisible to it). What gets logged is the RESOLVED bullet text, not your query, so the later `complete_action` or `drop_action` logs matching text and `current_focus` clears. Errors with `INTERNAL_ERROR` when `query` matches no open bullet, and on an ambiguous match (several bullets contain it) -- the message lists the candidates; re-call with enough text to pick one. For work that is NOT on the map yet, use `start_unplanned_action` instead: this tool will not create a bullet, deliberately, because a fallback would turn a typo into a new action silently."
    )]
