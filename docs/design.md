...
- **`<stem>.md` beside `<stem>/`** inside a portfolio: an evidence *stub* and the artefact folder it owns. Everything inside that folder is an attachment, whatever its extension and however deeply nested — a filed markdown document is a document, not a note, and is deliberately kept out of the index (it has no frontmatter contract; its searchable abstract lives in the stub). Ownership is resolved by walking a file’s ancestors, so the rule holds under any intervening folder.

  Two constraints keep the rule from swallowing real notes. **Ownership is established positively**: the sibling `<stem>.md` must actually parse as an evidence note carrying a `kind`, not merely exist — otherwise an ordinary note that happens to share a name with a hand-made grouping folder (`2026-Q2.md` beside `2026-Q2/`) would evict that folder’s notes from search, backlinks and portfolio contents, silently, with the files untouched on disk. And it **applies only inside `portfolios/`** — elsewhere a `.md` beside a folder of the same name is legitimate, notably a stewardship with both a flat and an expanded spelling.
...
**cdno-domain**: all RLM business logic. Defines the note type enum and per-type frontmatter schemas. Validates notes against schemas. Enforces rules (5-project cap, tasks must link to projects, a hard milestone must carry a date). Implements queries (commitments aggregation, portfolio staleness, active questions, weekly activity summary). Handles state transitions (park a project, complete a commitment, triage an inbox item). This is the heart of cuaderno.
...
>
> Enforcement reads from the *effective* template, not a hardcoded list, so customisation and enforcement agree: `cdno normalise` derives each note's canonical frontmatter order from its resolved template (a custom template that reorders or adds fields is honoured), and the daily "keep-last" section (#232) is the daily template's last heading (`## Logs` by default, or whatever a custom template ends with).
