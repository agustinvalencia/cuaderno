...
**Integrity self-check.** On startup, before reconciliation, the tool runs `PRAGMA integrity_check` on the SQLite database. If corruption is detected, it logs a warning and triggers a full reindex from the vault files. Since the vault files are the source of truth, this is always safe — the index is rebuilt in seconds and no data is lost.
