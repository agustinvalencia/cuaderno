...
**Concurrency.** `Vault` is synchronous (§4). Tool methods are async because `ServerHandler` is async, and every handler routes its vault calls through `CuadernoServer::with_vault`, which runs the synchronous domain work on tokio's blocking pool via `spawn_blocking` (GH #303, done) — one blocking task per request, so async workers never block on disk/SQLite. The HTTP binary keeps its transport-level concurrency cap plus request-body size limit as backpressure, and its reconciliation loop also runs on `spawn_blocking`.
