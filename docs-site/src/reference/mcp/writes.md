...
| Tool | Inputs | Effect |
|------|--------|--------|
| `file_to_portfolio` | `portfolio`, `source`, `origin`, `content?`, `attach?` | File evidence into a portfolio; `attach` is a server-side path to a non-Markdown artefact. ([`cdno file`](../cli/file.md)) |
...
| `add_action` | `project`, `title`, `energy`, `with_note?` | Append a next action; `with_note` also scaffolds a manifest note. |
| `promote_action` | `project`, `query` | Promote a bullet to a manifest note (substring match). |
...
| Tool | Inputs | Effect |
|------|--------|--------|
| `create_commitment` | `title`, `due`, `context`, `project?`, `stewardship?` | Create a standalone commitment note. |
| `complete_commitment` | `commitment` (slug) | Mark a commitment done and archive it. |
| `create_tracking_entry` | `stewardship`, `activity`, `routine?`, `content?` | File a tracking note under an expanded stewardship. |
