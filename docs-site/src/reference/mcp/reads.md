...
| `current_focus` | — | The action started and not yet closed, or `null`. Replayed from today's `## Logs`, so a start made from the CLI or by hand counts too; a completion or a drop clears it. The MCP form of [`cdno now`](../cli/now.md). |
