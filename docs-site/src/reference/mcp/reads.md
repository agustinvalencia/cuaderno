...
| `current_focus` | — | The action started and not yet closed, or `null`. Replayed from today's `## Logs`, so a start made from the CLI counts too, as does one written by hand in the log's own shape (`started [[slug]] — text`, em dash U+2014 — see [`cdno now`](../cli/now.md)); a completion or a drop clears it. The MCP form of [`cdno now`](../cli/now.md). |
