...
| `create_custom_note` | `type_name`, `title`, `fields?`, `vars?` | Create a note of a **config-defined** custom type (`[note_types.<name>]`); the built-ins each have their own tool above. `fields` is a name → value map of the type's declared frontmatter fields — every `required` one must be present, and each key must be declared. Call [`list_note_types`](reads.md) first to discover a vault's types and their fields. ([`cdno note create`](../cli/note.md)) |
